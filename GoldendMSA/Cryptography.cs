...
namespace Cryptography
{
    public class Md4 : HashAlgorithm
    {
        ...
        protected override void HashCore(byte[] array, int ibStart, int cbSize)
        {
            ...
            while (cbSize > 0)
            {
                var bytesToCopy = Math.Min(cbSize, 64 - _bufferLength);
        ...
        protected override byte[] HashFinal()
        {
            ...
            var bitLength = _totalLength * 8;
            for (var i = 0; i < 8; i++) _buffer[56 + i] = (byte)(bitLength >> (i * 8));
    ...
    public class Aes256
    {
        ...
        private byte[] StringToKey(byte[] password, string salt, int seedSize, int keySize, byte[] parameters = null)
        {
            ...
            var kerberosConstant = Encoding.UTF8.GetBytes("kerberos");
            var finalKey = Derive(tempKey, kerberosConstant, seedSize, keySize);
        ...
        {
            var plaintext = NFold(constant, BLOCKSIZE);

            var rndseed = new byte[0];
            while (rndseed.Length < seedSize)
            {
                var ciphertext = BasicEncrypt(key, plaintext);
                var newRndseed = new byte[rndseed.Length + ciphertext.Length];
        ...
        private static byte[] BasicEncrypt(byte[] key, byte[] plaintext)
        {
        ...
        private byte[] NFold(byte[] input, int nbytes)
        {
            ...
            var lcm = Lcm(nbytes, inputLen);

            var bigstr = new byte[lcm];
            for (var i = 0; i < lcm / inputLen; i++)
            {
                var rotated = RotateRight(input, 13 * i);
            ...
            for (var i = 0; i < lcm; i += nbytes)
            {
                var slice = new byte[nbytes];
        ...
        private byte[] RotateRight(byte[] data, int nbits)
        {
            ...
            var nbytes = nbits / 8 % data.Length;
            var remain = nbits % 8;

            for (var i = 0; i < data.Length; i++)
            {
                var sourceIndex = (i - nbytes + data.Length) % data.Length;
                var prevIndex = (sourceIndex - 1 + data.Length) % data.Length;
        ...
        private byte[] AddOnesComplement(byte[] a, byte[] b)
        {
            ...
            do
            {
                ...
                for (var i = 0; i < result.Length; i++)
                    ...
                    {
                        hasCarry = true;
                        var nextIndex = (i + 1) % result.Length;
                        result[nextIndex] += result[i] >> 8;
                        result[i] &= 0xFF;
                    }
            } while (hasCarry);

            var final = new byte[a.Length];
            for (var i = 0; i < a.Length; i++) final[i] = (byte)result[i];
