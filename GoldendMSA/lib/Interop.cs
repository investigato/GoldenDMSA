...
namespace GoldendMSA.lib
{
    public class Interop
    {
        ...
        public delegate int KERB_ECRYPT_Decrypt(IntPtr pContext, byte[] data, int dataSize, byte[] output,
            ref int outputSize);

        public delegate int KERB_ECRYPT_Encrypt(IntPtr pContext, byte[] data, int dataSize, byte[] output,
            ref int outputSize);

        public delegate int KERB_ECRYPT_Finish(ref IntPtr pContext);

        public delegate int KERB_ECRYPT_HashPassword(UNICODE_STRING Password, UNICODE_STRING Salt, int count,
            byte[] output);
        ...
        public enum KERBEROS_ERROR : uint
        {
            ...
            KDC_ERR_SVC_UNAVAILABLE =
                0x1D, // KDC is unavailable (modified as stated here: https://github.com/dotnet/Kerberos.NET/blob/develop/Kerberos.NET/Entities/Krb/KerberosErrorCode.cs)
            ...
            KDC_ERR_DH_KEY_PARAMETERS_NOT_ACCEPTED =
                0x41, // KDC policy has determined the provided Diffie-Hellman key parameters are not acceptable
            ...
            KDC_ERR_INCONSISTENT_KEY_PURPOSE =
                0x4D, // The client certificate does not contain the KeyPurposeId EKU and is required

            KDC_ERR_DIGEST_IN_CERT_NOT_ACCEPTED =
                0x4E, // The signature algorithm used to sign the CA certificate is not accepted
            ...
            KDC_ERR_DIGEST_IN_SIGNED_DATA_NOT_ACCEPTED =
                0x50, // The signature algorithm used to sign the request is not accepted

            KDC_ERR_PUBLIC_KEY_ENCRYPTION_NOT_SUPPORTED =
                0x51, // The KDC does not support public key encryption for PKINIT
            ...
            KDC_ERR_MORE_PREAUTH_DATA_REQUIRED =
                0x5B, // The KDC found the presented pre-auth data incomplete and requires additional information

            KDC_ERR_PREAUTH_BAD_AUTHENTICATION_SET =
                0x5C, // The client sent an authentication set that the KDC was not expecting

            KDC_ERR_UNKNOWN_CRITICAL_FAST_OPTIONS =
                0x5D // The provided FAST options that were marked as critical are unknown to the KDC and cannot be processed
        }
            ...
            public SecBufferDesc(MultipleSecBufferHelper[] secBufferBytesArray)
            {
                ...
                for (var Index = 0; Index < secBufferBytesArray.Length; Index++)
                {
                    ...
                    Marshal.WriteIntPtr(pBuffers,
                        CurrentOffset + Marshal.SizeOf(ThisSecBuffer.cbBuffer) +
                        Marshal.SizeOf(ThisSecBuffer.BufferType), ThisSecBuffer.pvBuffer);
                }
            }
