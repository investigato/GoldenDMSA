 ...
 */
    ...
    public class AsnElt
    {
        ...
        private static readonly IComparer<byte[]> COMPARER_LEXICOGRAPHIC =
        ...
        public int ValueLength
        {
            get
            {
                if (valLen < 0)
                {
                    ...
                    {
                        var vlen = 0;
                        foreach (var a in Sub) vlen += a.EncodedLength;
                        valLen = vlen;
                    }
                    else
                    {
        ...
        {
            if (TagClass != tc || TagValue != tv) throw new AsnException("unexpected tag: " + TagString);
        }
        ...
        private static AsnElt DecodeNoCopy(byte[] buf, int off, int len)
        {
            ...
            if (cons)
            {
                var subs = new List<AsnElt>();
                off = valOff;
                var lim = valOff + valLen;
                while (off < lim)
                {
                    var b = DecodeNoCopy(buf, off, lim - off);
                    off += b.objLen;
                    subs.Add(b);
                }
        ...
        {
            if (off < 0) throw new AsnException("invalid value offset: " + off);
            if (objBuf == null)
            {
                var k = 0;
                foreach (var a in Sub)
                {
                    var slen = a.EncodedLength;
                    if (k + slen > off) return a.ValueByte(off - k);
                }
            }
            else
            {
                if (off < valLen) return objBuf[valOff + off];
            }
        ...
        {
            return Encode(0, int.MaxValue, dst, off);
        }
        ...
        private int Encode(int start, int end, byte[] dst, int dstOff)
        {
            ...
            if (hasEncodedHeader)
            {
                var from = objOff + Math.Max(0, start);
                var to = objOff + Math.Min(objLen, end);
                var len = to - from;
                if (len > 0)
                {
             ...
             * Encode tag.
             */
            var fb = (TagClass << 6) + (Constructed ? 0x20 : 0x00);
            if (TagValue < 0x1F)
            {
                fb |= TagValue & 0x1F;
            ...
            else
            {
                ...
                off++;
                var k = 0;
                for (var v = TagValue; v > 0; v >>= 7, k += 7) ;
                while (k > 0)
                {
                    k -= 7;
                    var v = (TagValue >> k) & 0x7F;
                    if (k != 0) v |= 0x80;
        ...
        {
            return EncodeValue(0, int.MaxValue, dst, off);
        }
        ...
        public long GetInteger()
        {
            ...
            var vlen = ValueLength;
            if (vlen == 0) throw new AsnException("invalid INTEGER (length = 0)");
            var v = ValueByte(0);
            long x;
            if ((v & 0x80) != 0)
            {
        ...
        {
            var v = GetInteger();
            if (v < min || v > max) throw new AsnException("integer out of allowed range");
            return v;
        }
        ...
        public string GetIntegerHex()
        {
            ...
            var k = 0;
            while (k < tmp.Length && tmp[k] == 0) k++;
            if (k == tmp.Length) return "0x00";
            sb.Append("0x");
            while (k < tmp.Length) sb.AppendFormat("{0:X2}", tmp[k++]);
            return sb.ToString();
        }
        ...
        public int GetOctetString(byte[] dst, int off)
        {
            ...
            if (dst != null) return CopyValue(dst, off);
        ...
        public byte[] GetBitString(out int bitLength)
        {
            ...
            var r = new byte[vlen - 1];
            CopyValueChunk(1, vlen - 1, r, 0);
            if (vlen > 1) r[r.Length - 1] &= (byte)(0xFF << fb);
        ...
        {
            CheckPrimitive();
            if (valLen == 0) throw new AsnException("zero-length OID");
            int v = objBuf[valOff];
            if (v >= 120)
                throw new AsnException(
        ...
        {
            var tc = new char[len];
            for (var i = 0; i < len; i++) tc[i] = (char)buf[off + i];
            VerifyChars(tc, type);
            ...
            for (var k = 0; k < 2; k++)
            {
                ...
                for (var i = 0; i < len; i++)
                {
                    ...
                    while (e-- > 0)
                    {
                        ...
                        int d = buf[off + i];
                        if (d < 0x80 || d > 0xBF) throw BadByte(d, UTF8String);
                        c = (c << 6) + (d & 0x3F);
                    }

                    if (c > 0x10FFFF) throw BadChar(c, UTF8String);
                    if (c > 0xFFFF)
                    {
                        c -= 0x10000;
                        var hi = 0xD800 + (c >> 10);
                    ...
                    {
                        if (tc != null) tc[tcOff] = (char)c;
                        tcOff++;
                    }
                }

                if (tc == null) tc = new char[tcOff];
            }
            ...
            for (var k = 0; k < 2; k++)
            {
                ...
                for (var i = 0; i < len; i++)
                {
                    ...
                    if (be)
                        ...
                        c = (b3 << 24) | (b2 << 16)
                                       | (b1 << 8) | b0;
                    if (c > 0x10FFFF) throw BadChar((int)c, UniversalString);
                    if (c > 0xFFFF)
                    {
                        c -= 0x10000;
                        var hi = 0xD800 + (int)(c >> 10);
                        var lo = 0xDC00 + (int)(c & 0x3FF);
                        if (tc != null)
                        {
                            tc[tcOff] = (char)hi;
                            tc[tcOff + 1] = (char)lo;
        ...
        private static void VerifyChars(char[] tc, int type)
        {
            ...
            {
                int c = tc[i];
                if (c >= 0xFDD0 && c <= 0xFDEF) throw BadChar(c, type);
                if (c == 0xFFFE || c == 0xFFFF) throw BadChar(c, type);
                if (c < 0xD800 || c > 0xDFFF) continue;
                if (c > 0xDBFF) throw BadChar(c, type);
                var hi = c & 0x3FF;
                if (++i >= tc.Length) throw BadChar(c, type);
                c = tc[i];
                if (c < 0xDC00 || c > 0xDFFF) throw BadChar(c, type);
                var lo = c & 0x3FF;
                c = 0x10000 + lo + (hi << 10);
                if ((c & 0xFFFE) == 0xFFFE) throw BadChar(c, type);
            }
        }
        ...
        public DateTime GetTime(int type)
        {
            ...
            else
            {
                ...
                {
                    var t = s.Substring(j + 1);
                    s = s.Substring(0, j);
                    if (t.Length != 4) throw BadTime(type, orig);
             ...
             */
            if (noTZ && !isGen) throw BadTime(type, orig);

            /*
             * Parse the date elements.
             */
            if (s.Length < 4) throw BadTime(type, orig);
            var year = Dec2(s, 0, ref good);
            if (isGen)
            {
            ...
            var month = Dec2(s, 0, ref good);
            var day = Dec2(s, 2, ref good);
            var hour = Dec2(s, 4, ref good);
            var minute = Dec2(s, 6, ref good);
            var second = 0;
            var millisecond = 0;
            if (isGen)
            {
             ...
             */
            if (!good) throw BadTime(type, orig);
            ...
            try
            {
                ...
                var tzOff = new TimeSpan(tzHours, tzMinutes, 0);
                if (negZ) tzOff = tzOff.Negate();
        ...
        {
            var tt = type == UTCTime ? "UTCTime" : "GeneralizedTime";
            var msg = string.Format("invalid {0} string: '{1}'", tt, s);
            if (e == null) return new AsnException(msg);
        ...
        public static AsnElt MakeInteger(long x)
        {
            ...
            var k = 1;
            for (var w = x; w <= -(long)0x80; w >>= 8) k++;
            var v = new byte[k];
            for (var w = x; k > 0; w >>= 8) v[--k] = (byte)w;
            ...
            byte[] buf, int off, int len)
        {
            ...
            Array.Copy(buf, off, tmp, 1, len);
            if (len > 0) tmp[len - 1] &= (byte)(0xFF << unusedBits);
            return MakePrimitiveInner(BIT_STRING, tmp);
        }
        ...
        public static AsnElt MakeSetOf(params AsnElt[] subs)
        {
            ...
            else
            {
                ...
                foreach (var ax in subs) d[ax.Encode()] = ax;
                var tmp = new AsnElt[d.Count];
                var j = 0;
                foreach (var ax in d.Values) tmp[j++] = ax;
                a.Sub = tmp;
            }
        ...
        public static AsnElt MakeOID(string str)
        {
            ...
            for (var i = 0; i < n; i++)
            {
                ...
                if (x < 0)
                    x = 0;
                else if (x > (long.MaxValue - 9) / 10) throw new AsnException("OID element overflow");
                x = x * 10 + (c - '0');
            }
        ...
        private static byte[] EncodeMono(string str)
        {
            ...
            var k = 0;
            foreach (var c in str) r[k++] = (byte)c;
            return r;
        }
