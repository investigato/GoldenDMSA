    ...
    public class AS_REP
    {
        ...
        private void Decode(AsnElt asn_AS_REP)
        {
            ...
            if (asn_AS_REP.TagValue != (int)Interop.KERB_MESSAGE_TYPE.AS_REP)
                throw new Exception("AS-REP tag value should be 11");

            if (asn_AS_REP.Sub.Length != 1 || asn_AS_REP.Sub[0].TagValue != 16)
                throw new Exception("First AS-REP sub should be a sequence");

            // extract the KDC-REP out
            var kdc_rep = asn_AS_REP.Sub[0].Sub;
            padata = new List<PA_DATA>();

            foreach (var s in kdc_rep)
                switch (s.TagValue)
                {
                    case 0:
                    ...
                    case 2:
                        // sequence of pa-data
                        int i;
                        for (i = 0; i < s.Sub[0].Sub.Length; i++) padata.Add(new PA_DATA(s.Sub[0].Sub[i]));
                        break;
                    case 3:
